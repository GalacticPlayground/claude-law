...
Top-level refresh outputs also live under `references/`:

- `references/in_statutes/` — 27 Indiana Code article files generated by `scripts/pull_indiana_statutes.py`.
- `references/in_court_rules/` — 7 Indiana court-rule files generated by `scripts/pull_indiana_rules.py`.

Last attempted refresh: 2026-06-04 with `HTTPS_PROXY=http://192.168.8.21:9091`. The current workspace sandbox denied outbound sockets before the proxy connection could be established (`Operation not permitted`), so the generated files are fetch-failure placeholders rather than populated primary-law text. Rerun the pullers from a network-enabled environment to replace these placeholders with the full corpus.
