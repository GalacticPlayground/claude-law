...
Update `USC_RELEASE` and `ECFR_AS_OF` constants in the script when refreshing. The shared `us-federal-debt-corpus` plugin is the canonical home for this corpus — every state plugin reaches it via a symlink, so a single pull refreshes content for all states. The same script also populates the sibling `federal-bankruptcy/` corpus.
