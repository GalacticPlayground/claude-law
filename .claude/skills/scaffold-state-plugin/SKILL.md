...
- Writes stub `SKILL.md` files for each of the 21 skills with
  ...
  a category-appropriate **NOT LEGAL ADVICE disclaimer**
  blockquote (drafting scaffolders identify the user as
  decision-maker; declaration scaffolders identify the user
  as declarant; order scaffolders flag that only a judge
  signs; pro-se skills add a consult-an-attorney prompt;
  subject-matter bundles flag fact-specificity), and a TODO
  list of sections to author. Authors expanding a stub MUST
  preserve the disclaimer block.
...
1. **Line-numbered pleading paper (default ON)** — apply
   continuous line numbering down the left margin of every
   generated motion, memorandum, declaration / affidavit,
   notice, and proposed order. Exhibits and attachments are
   exempt. Some states require it (CA — CRC 2.108(b)); some
   states make it convention (WA, OR); some states do not
   adopt it as local convention (CO, IN, NY, OH); but the
   marketplace ships it ON everywhere because (a) it never
   produces a non-compliant filing and (b) it enables
   page+line pinpoint citation that the court and opposing
   counsel routinely use. The technical recipe is identical
   in every state plugin:
   ...
   Do NOT set `start: 1` explicitly — it renders off by one
   in LibreOffice (used as the headless PDF renderer). Omit
   `start`; it defaults to 1 in OOXML.

   In states with paragraph-numbering mechanics (e.g., NY
   CPLR 3014; IN T.R. 10(B)), paragraph numbering is a
   distinct content mechanic and stays ON in addition to
   line numbering.

2. **Footer with title + Page X of Y (default ON)** — every
   generated document carries a footer on every page with a
   short document identifier on the left and `Page X of Y`
   on the right (using Word's `PAGE` and `NUMPAGES` fields,
   not static numbers). The footer runs continuously from
   page 1 through the last exhibit page — never restart or
   suppress.

3. **Two-section pattern for page-1 top margin (where the
   state's rule sets a larger top margin on page 1)** — when
   the state's rule sets a different top margin on page 1
   than on subsequent pages (e.g., WA GR 14's 3-inch top
   margin on page 1, CA CRC 2.110's 2.5-inch top margin),
   either use two sections OR add a leading paragraph of
   `spacing: { before: ... }` to push page-1 content down.
   When using two sections, ANY section-level property
   (notably `lineNumbers`) must be set on BOTH sections —
   otherwise line numbering stops at the section break.

When authoring the new state's `<abbr>-statewide-format`
SKILL.md, include a "Line numbering (pleading paper)"
section with the recipe above and explicit "Apply it **by
default**" language. Do not frame the convention as opt-in.
See any existing state's `<abbr>-statewide-format/SKILL.md`
for the canonical layout (CO / IN / NY / OH adopted the
default-ON form most recently and are the cleanest models).
